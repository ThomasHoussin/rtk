...
use crate::tracking;
use crate::tsc_cmd;
use crate::utils::{script_cmd, strip_ansi};
use crate::vitest_cmd;
...
lazy_static::lazy_static! {
    ...
    static ref RE_SCRIPT_ECHO: Regex = Regex::new(r"^\$ (yarn|node|vitest|tsc|eslint|prettier|npm|pnpm)").expect("RE_SCRIPT_ECHO regex");
...
/// This is speculative: `test:e2e` may actually run playwright, not vitest.
/// If the filter doesn't match the actual output, `apply_identified_filter` returns
/// empty and the caller falls back to passthrough — no data is lost.
fn identify_filter(script_name: &str) -> Option<&'static str> {
    let key = match script_name.split_once(':') {
...
Another real line";
    ...
    fn test_identify_compound_test_routes_vitest() {
        assert_eq!(identify_filter("test:e2e"), Some("vitest"));
