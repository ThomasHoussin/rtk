use crate::tracking;
use crate::utils::{script_cmd, strip_ansi, truncate};
...
fn cdk_command() -> Command {
    if crate::utils::has_program("cdk") {
        script_cmd("cdk")
    } else {
...
pub fn run_diff(args: &[String], verbose: u8) -> Result<()> {
    ...
    if let Some(hint) = crate::tee::tee_and_hint(&raw, "cdk_diff", exit_code) {
        println!("{}\n{}", filtered, hint);
    } else {
        println!("{}", filtered);
    }
...
pub fn run_synth(args: &[String], verbose: u8) -> Result<()> {
    ...
    if let Some(hint) = crate::tee::tee_and_hint(&raw, "cdk_synth", exit_code) {
        println!("{}\n{}", filtered, hint);
    } else {
        println!("{}", filtered);
    }
...
pub fn run_deploy(args: &[String], verbose: u8) -> Result<()> {
    ...
    if let Some(hint) = crate::tee::tee_and_hint(&raw, "cdk_deploy", exit_code) {
        println!("{}\n{}", filtered, hint);
    } else {
        println!("{}", filtered);
    }
